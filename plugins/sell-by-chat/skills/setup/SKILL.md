...
1. Ask where their files live (or find them): `VOICE-DNA.md` and any ICP / business-brain file, in the current folder or home folder. **Missing one or both? Never block.** Follow `${CLAUDE_PLUGIN_ROOT}/references/voice-dna-quickstart.md`: pull from their Purely Personal brain if reachable, build from pasted real writing, or run the 5-question interview. Ten minutes, then continue.
